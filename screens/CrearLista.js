...
const CrearLista = () => {
  ...
  const obtenerCategorias = async () => {
    try {
      ...
      const categoriasData = categoriasSnapshot.docs.map(
        (doc) => doc.data().nombreCategoria
      );
    ...
    try {
      ...
      if (productoSnapshot.exists()) {
        ...
        if (mostrarAlerta) {
          Alert.alert(
            "Producto encontrado",
            `El producto '${nombreProducto}' ya existe en la base de datos.`
          );
      ...
      Alert.alert(
        "Error",
        "Hubo un error al buscar el producto en la base de datos."
      );
    }
  };
  ...
  return (
    <ScrollView
      ...
      refreshControl={
        <RefreshControl refreshing={refreshing} onRefresh={obtenerCategorias} />
      }
    >
      <View style={styles.inputContainer}>
        <Text>ID: {state.idProducto}</Text>
